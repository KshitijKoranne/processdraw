...
function auditDetails(detailsRaw?: string) {
  if (!detailsRaw) return "";
  try {
    const details = JSON.parse(detailsRaw);
    const parts: string[] = [];
    if (details.revisionNumber !== undefined && details.revisionNumber !== null) parts.push(`Rev ${details.revisionNumber}`);
    ...
    if (details.isFirstUser) parts.push("First user (auto IT Admin)");
    if (details.employeeCode) parts.push(`Code: ${details.employeeCode}`);
    if (details.finalizedReset) parts.push("finalized reset");
    if (details.remarks) parts.push(`Remarks: ${details.remarks}`);
    if (details.comment) parts.push(`Comment: ${details.comment}`);
    return parts.join(" · ");
  } catch {
    return detailsRaw;
  }
...
function CreateEmployeeForm({ onSuccess }: { onSuccess: (msg: string) => void }) {
  ...
  const handleCreate = async () => {
    setError("");
    if (!employeeCode.trim() || !fullName.trim() || !password.trim()) return setError("All fields are required");
    if (password.length < 8) return setError("Password must be at least 8 characters");
    setLoading(true);
    try {
      const res = await fetch("/api/create-employee", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ employeeCode: employeeCode.trim(), fullName: fullName.trim(), password, role }) });
      const data = await res.json();
      if (!res.ok) setError(data.error || "Failed to create employee");
      else { onSuccess(`Employee ${employeeCode} created. They can now sign in with their employee code and password.`); setEmployeeCode(""); setFullName(""); setPassword(""); setRole("user"); }
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "28px 28px", marginBottom: 28 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
        <div><label style={{ fontSize: 11, color: C.textMuted, display: "block", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>Employee Code</label><input value={employeeCode} onChange={(e) => setEmployeeCode(e.target.value)} placeholder="e.g. EMP001" style={inputS} /></div>
        <div><label style={{ fontSize: 11, color: C.textMuted, display: "block", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>Full Name</label><input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="e.g. Rahul Sharma" style={inputS} /></div>
        <div><label style={{ fontSize: 11, color: C.textMuted, display: "block", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>Temporary Password</label><input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 8 characters" type="password" style={inputS} /></div>
        <div><label style={{ fontSize: 11, color: C.textMuted, display: "block", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>Role</label><select value={role} onChange={(e) => setRole(e.target.value)} style={{ ...inputS, cursor: "pointer" }}>{ROLES.map((r) => (<option key={r.value} value={r.value}>{r.label}</option>))}</select></div>
      </div>
      {error && <div style={{ fontSize: 12, color: C.danger, marginBottom: 12, padding: "8px 12px", background: "#fdf0ee", borderRadius: 6 }}>{error}</div>}
      <div style={{ display: "flex", justifyContent: "flex-end" }}><button onClick={handleCreate} disabled={loading} style={{ ...btnS(C.accent, "#fff"), opacity: loading ? 0.6 : 1, padding: "10px 28px" }}>{loading ? "Creating..." : "Create Employee"}</button></div>
      <div style={{ fontSize: 11, color: C.textLight, marginTop: 12, lineHeight: 1.5 }}>The employee will sign in using their employee code as username and the password you set. They should change their password after first login.</div>
    </div>
  );
}
...
export default function AdminPanel({ onBack, isFullScreen }: { onBack: () => void; isFullScreen?: boolean }) {
  ...
  const handleRoleChange = async (userId: any, role: string) => { setUpdating(userId); try { await updateRole({ userId, role }); } catch (e) { console.error(e); } setUpdating(null); };
  const handleToggleDisabled = async (userId: any, disabled: boolean) => { setUpdating(userId); try { await toggleDisabled({ userId, disabled }); } catch (e) { console.error(e); } setUpdating(null); };
