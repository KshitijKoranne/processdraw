...
export function DiagramCanvas({ svgRef, layout, blocks, annotations, readOnly, onEditBlock, onDeleteBlock, onAddBlock, onFinalize, onPickSide, onPickBetween, onToggleSideArrow }: {
  ...
  layout.positions.forEach((pos: any, index: number) => {
    ...
    pos.lines.forEach((line: string, lineIndex: number) => output.push(<text key={`${pos.block.id}-${lineIndex}`} x={pos.blockX + pos.width / 2} y={pos.blockY + pos.height / 2 - ((pos.lines.length - 1) * 18) / 2 + lineIndex * 18} textAnchor="middle" dominantBaseline="middle" fontFamily={CHART_FONT} fontSize={CHART_FONT_SIZE} fontWeight="700" fill={COLORS.ink}>{line}</text>));
    ...
    if (index < layout.positions.length - 1) {
      ...
      (["left", "right"] as Side[]).forEach((side) => (ann[side] || []).forEach((item: any, annIndex: number) => {
        ...
        dimensions.lines.forEach((line, lineIndex) => output.push(<text key={`ann-${side}-${index}-${annIndex}-${lineIndex}`} x={textX} y={textY + lineIndex * 14} textAnchor={side === "left" ? "end" : "start"} fontFamily={CHART_FONT} fontSize={CHART_FONT_SIZE}>{line}</text>));
      }));
    ...
    const drawSide = (items: any[], side: Side) => items.forEach((itemPos) => {
      ...
      output.push(<g key={`side-arrow-${itemPos.item.id}`} className={!readOnly ? "pd-ctl" : undefined} onClick={(event: any) => { if (readOnly) return; event.stopPropagation(); onToggleSideArrow(pos.block.id, side, itemPos.item.id); }}><line x1={leftX} x2={rightX} y1={itemPos.centerY} y2={itemPos.centerY} stroke={COLORS.ink} strokeWidth={2} /><polygon points={itemPos.item.arrowDir === "right" ? `${headX},${itemPos.centerY} ${headX - ARROW_SIZE},${itemPos.centerY - 4} ${headX - ARROW_SIZE},${itemPos.centerY + 4}` : `${headX},${itemPos.centerY} ${headX + ARROW_SIZE},${itemPos.centerY - 4} ${headX + ARROW_SIZE},${itemPos.centerY + 4}`} fill={COLORS.ink} />{!readOnly && <line x1={leftX} x2={rightX} y1={itemPos.centerY} y2={itemPos.centerY} stroke="transparent" strokeWidth={18} />}</g>);
    });
    ...
    if (index === layout.positions.length - 1 && !readOnly) output.push(<g key="end-row"><Plus x={CANVAS_CENTER_X - 30} y={pos.blockY + pos.height + 36} onClick={onAddBlock} /><g className="pd-ctl" onClick={onFinalize}><rect x={CANVAS_CENTER_X + 6} y={pos.blockY + pos.height + 26} width="52" height="22" rx="5" fill="#fff" stroke={COLORS.danger} /><text x={CANVAS_CENTER_X + 32} y={pos.blockY + pos.height + 41} textAnchor="middle" fontFamily={CHART_FONT} fontSize={CHART_FONT_SIZE} fontWeight="700" fill={COLORS.danger}>END</text></g></g>);
  });

  return <svg ref={svgRef} className="pd-svg" xmlns="http://www.w3.org/2000/svg" width={SVG_WIDTH} height={layout.height} viewBox={`0 0 ${SVG_WIDTH} ${layout.height}`} fontFamily={CHART_FONT} fontSize={CHART_FONT_SIZE}>{output}</svg>;
}
