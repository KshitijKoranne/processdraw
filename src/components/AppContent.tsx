...
function LandingPage() {
  ...
  const features = [
    ...
    { icon: "📄", title: "Print-Ready Export", desc: "Export as high-resolution PNG or PDF, automatically split at A4 page boundaries — ready to drop straight into your documents." },
    { icon: "✍️", title: "Document-Ready Footer", desc: "Every page includes Prepared by / Checked by signature lines and page numbers. Unapproved exports carry a watermark." },
  ...
  return (
    <div className="lp-root">
      ...
      <section className="lp-section alt"><div className="lp-shell lp-demo-wrap"><div className="lp-section-heading"><h2>Try It Yourself</h2><p>Explore ProcessDraw with a demo account — no sign-up required. Demo data resets every few hours.<br />Password for all demo accounts: <strong>demo1234</strong></p></div><div className="lp-grid lp-demos">{demos.map((demo) => <div key={demo.code} className="lp-demo-card"><div className="lp-demo-role" style={{ color: demo.color }}>{demo.role}</div><div className="lp-demo-desc">{demo.desc}</div><div className="lp-demo-user">Username: <strong style={{ color: C.text }}>{demo.code}</strong></div><a href="/sign-in" style={{ textDecoration: "none" }}><button className="lp-demo-btn" style={{ background: demo.color }}>Try as {demo.role}</button></a></div>)}</div></div></section>
      <section className="lp-shell lp-cta-section"><div className="lp-cta-box"><h2>Ready to simplify your process documentation?</h2><p>Your administrator creates your account. Sign in with your employee credentials to get started.</p><a href="/sign-in" style={{ textDecoration: "none" }}><PrimaryButton>Sign In</PrimaryButton></a></div></section>
