...
export const updateUserRole = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!identity) throw new Error("Not authenticated");
    const currentUser = await ctx.db.query("users").withIndex("by_clerk_id", (q) => q.eq("clerkId", identity.subject)).unique();
    if (!currentUser || currentUser.role !== "it_admin" || currentUser.disabled) throw new Error("Only active IT Admins can change roles");
    if (currentUser.isDemo) throw new Error("Demo admins cannot change user roles");
    ...
    if (targetUser.isDemo) throw new Error("Demo users are managed by demo reset only");
...
export const toggleDisabled = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!identity) throw new Error("Not authenticated");
    const currentUser = await ctx.db.query("users").withIndex("by_clerk_id", (q) => q.eq("clerkId", identity.subject)).unique();
    if (!currentUser || currentUser.role !== "it_admin" || currentUser.disabled) throw new Error("Only active IT Admins can disable/enable users");
    if (currentUser.isDemo) throw new Error("Demo admins cannot disable users");
    const targetUser = await ctx.db.get(args.userId);
    if (!targetUser) throw new Error("User not found");
...
export const preRegister = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logAction(ctx, {
      ...
      details: JSON.stringify({ role: args.role, employeeCode: args.employeeCode, isDemo: args.isDemo || false }),
    });
    return id;
  },
});
